...
class DatasetLog:
    ...
    def _log_params(self):
        from encoder import hyperparameters
        self.write_line("Parameter values:")
        for param_name in (p for p in dir(hyperparameters) if not p.startswith("__")):
            value = getattr(hyperparameters, param_name)
...
def _preprocess_speaker_dirs(speaker_dirs, dataset_name, datasets_root, out_dir, extension,
    ...
    def preprocess_speaker(speaker_dir: Path):
        ...
        # There's a a possibility that the preprocessing was interrupted earlier, check if
...
# TODO: Implement preprocess functions for my datasets
...
def preprocess_librispeech(datasets_root: Path, out_dir: Path, skip_existing=False):
    for dataset_name in librispeech_datasets["train"]["clean"]:
        ...
        _preprocess_speaker_dirs(speaker_dirs, dataset_name, dataset_root, out_dir, "flac",
                                 skip_existing, logger)